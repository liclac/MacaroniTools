...
/**
 * \private Internal Parameter Typedef used by the Glossy Buttons
 */
typedef struct
{
	float color[4];			///< Color
	float caustic[4];		///< Caustic
	float expCoefficient;	///< Exp Coefficient
	float expOffset;		///< Exp Offset
	float expScale;			///< Exp Scale
	float initialWhite;		///< Initial White
	float finalWhite;		///< Final White
} GlossyParams;
...
///Perceptual Gloss Fraction for Color
...
///Perceptual Caustic Color for Color
...
///Set the Background of an UIButton to a glossy hue of the specified color
/**
 * This is essentially the only function you need to call directly:
 * the function to draw a glossy background on the button, optionally with a border
 */
